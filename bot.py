...
# Solo se permite la familia INDECISIÓN y una única operación global por vela.
REQUIRE_FORCE = False
...
LAST_GLOBAL_TRADE_CANDLE: Optional[int] = None
...
def analyze_closed_candle(pair: str, expected_closed_ts: int) -> bool:
    ...
    if result.get("entry_type") != "indecision" or signal not in ("call", "put"):
        return True
    ...
    pending = {
        "signal": signal,
        "score": score,
        "entry_quality": int(result.get("entry_quality", analysis.get("entry_quality", score)) or 0),
        "entry_type": result.get("entry_type"),
...
def process_pair(pair: str, allow_execution: bool = False) -> None:
    if IQ is None:
        return
...
def analyze_all_pairs() -> None:
    global LAST_GLOBAL_TRADE_CANDLE
    ...
    # Primero analiza todos los pares; no ejecuta ninguna señal todavía.
    for pair in list(PAIRS):
        if not BOT_RUNNING:
            return
        try:
            process_pair(pair, allow_execution=False)
        except Exception:
            logger.exception("Error procesando %s", pair)

    # Reúne las señales INDECISIÓN listas para N+1 y escoge solo una.
    if LAST_GLOBAL_TRADE_CANDLE == current_ts:
        return

    candidates = []
    with STATE_LOCK:
        for pair, pending in PENDING_ENTRY.items():
            if str(pending.get("entry_type")) != "indecision":
                continue
            if int(pending.get("execution_ts", -1)) != current_ts:
                continue
            signal = str(pending.get("signal", ""))
            if signal not in ("call", "put"):
                continue
            score = int(pending.get("score", 0) or 0)
            quality = int(pending.get("entry_quality", score) or 0)
            candidates.append((score, quality, pair, dict(pending)))

    if not candidates:
        return

    # Prioridad: score, calidad y orden alfabético estable como desempate.
    candidates.sort(key=lambda item: (-item[0], -item[1], item[2]))
    _, _, best_pair, best_pending = candidates[0]

    if execute_sniper(best_pair, best_pending):
        LAST_GLOBAL_TRADE_CANDLE = current_ts
        with STATE_LOCK:
            # Evita que otras señales de la misma vela se ejecuten después.
            for pair in list(PENDING_ENTRY):
                if pair != best_pair:
                    other = PENDING_ENTRY.get(pair, {})
                    if int(other.get("execution_ts", -1)) == current_ts:
                        PENDING_ENTRY.pop(pair, None)
...
def main() -> None:
    ...
    logger.info("========================================")
    logger.info("BOT BINARY OTC | INDECISIÓN | 1 MEJOR ENTRADA | N+1")
