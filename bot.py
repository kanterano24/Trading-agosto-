...
def _append_signal_record(record: Dict[str, Any]) -> None:
    """Guarda cada análisis para comparar familias de señales posteriormente."""
    fields = [
        "observed_at", "pair", "candle_timestamp", "execution_timestamp",
        "signal", "direction", "entry_type", "signal_family", "score",
        "entry_quality", "structure", "impulse_phase", "zone", "status",
        "reason",
    ]
    try:
        ...
        logger.warning("No se pudo guardar señal en %s: %s", SIGNAL_LOG_PATH, exc)


def _signal_family(result: Dict[str, Any]) -> str:
    """Identificador legible de la familia que produjo la señal."""
    entry_type = str(result.get("entry_type") or "").strip()
    labels = {
        "force": "fuerza",
        "continuity": "continuidad",
        "rest": "descanso",
        "indecision": "indecisión",
        "divergence": "divergencia_rsi",
        "rejection_support": "rechazo_soporte",
        "rejection_resistance": "rechazo_resistencia",
    }
    if entry_type:
...
def analyze_closed_candle(
    ...
    family = _signal_family(result)
    status = "signal" if signal in ("call", "put") else "sin_senal"

    # Registrar TODAS las familias sin imponer el filtro de FUERZA.
    # En modo observación ninguna señal se convierte en una orden.
    ...
    # ========================================================
    # TELEGRAM: TODAS LAS SEÑALES CLASIFICADAS
    # ========================================================
    ...
    telegram_send(
        ...
        f"Score: {score}/100\n"
        f"Calidad: {entry_quality}/100\n"
        f"Estructura: {analysis.get('structure')}\n"
        f"Fase impulso: {analysis.get('impulse_phase')}\n"
        f"Zona: {analysis.get('zone', result.get('zone'))}\n"
        f"N cierre: {expected_closed_ts}\n"
        f"N+1 objetivo: {expected_closed_ts + TIMEFRAME}\n\n"
        f"{result.get('reason', '')}"
    )
