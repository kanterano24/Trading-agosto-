...
def analyze_market(
    ...
    if structure == "bullish":
        ...
        # Para evitar las entradas observadas en las imágenes,
        # CALL solo se permite después de un rechazo real de soporte.
        # Continuidad, descanso, fuerza o divergencia por sí solos
        # nunca autorizan una entrada.
        if not rejection:
            result["reason"] = (
                "CALL bloqueado: no existe rechazo confirmado de soporte"
            )
            return result

        if not _rejection_has_confirmation(c, p, "bullish"):
            result["reason"] = (
    ...
    # Para evitar las entradas observadas en las imágenes,
    # PUT solo se permite después de un rechazo real de resistencia.
    # Continuidad, descanso, fuerza o divergencia por sí solos
    # nunca autorizan una entrada.
    if not rejection:
        result["reason"] = (
            "PUT bloqueado: no existe rechazo confirmado de resistencia"
        )
        return result

    if not _rejection_has_confirmation(c, p, "bearish"):
        result["reason"] = (
