...
# ============================================================
# FILTRO DE PRECISION: BOLLINGER + RSI + ATR TRAILING STOPS
# ============================================================
BB_PERIOD = 20
BB_STD = 2.0
ATR_TRAILING_MULTIPLIER = 2.0
PRECISION_MIN_RSI_CALL = 42.0
PRECISION_MAX_RSI_CALL = 67.0
PRECISION_MIN_RSI_PUT = 33.0
PRECISION_MAX_RSI_PUT = 58.0
...
def _apply_precision_filter(
    ...
    """Confirma tendencia, ubicación, RSI y ATR antes de exponer la señal."""
    ...
    if data.empty or len(data) < max(MIN_BARS, BB_PERIOD + 2):
        return result
    ...
    data["bb_mid"] = basis
    data["bb_upper"] = basis + BB_STD * deviation
    data["bb_lower"] = basis - BB_STD * deviation

    atr_series = data["tr"].rolling(ATR_PERIOD, min_periods=ATR_PERIOD).mean()
    trail = pd.Series(index=data.index, dtype=float)
    position = pd.Series(index=data.index, dtype=int)
    for i in range(len(data)):
        a = atr_series.iloc[i]
        price = close.iloc[i]
        if not np.isfinite(a) or a <= 0 or not np.isfinite(price):
            continue
        loss = float(a) * ATR_TRAILING_MULTIPLIER
        if i == 0 or not np.isfinite(trail.iloc[i - 1]):
            trail.iloc[i] = price - loss
            position.iloc[i] = 1
            continue
        previous_trail = float(trail.iloc[i - 1])
        previous_price = float(close.iloc[i - 1])
        if price > previous_trail and previous_price > previous_trail:
            trail.iloc[i] = max(previous_trail, price - loss)
        elif price < previous_trail and previous_price < previous_trail:
            trail.iloc[i] = min(previous_trail, price + loss)
        elif price > previous_trail:
            trail.iloc[i] = price - loss
        else:
            trail.iloc[i] = price + loss
        position.iloc[i] = 1 if price > trail.iloc[i] else -1

    cur, prev = data.iloc[-1], data.iloc[-2]
    v = {
        "ema_bullish": bool(cur["ema9"] > cur["ema21"] > cur["ema50"]),
        "ema_bearish": bool(cur["ema9"] < cur["ema21"] < cur["ema50"]),
        "rsi": _safe_float(cur.get("rsi"), 50.0),
        "close": _safe_float(cur.get("close"), np.nan),
        "bb_mid": _safe_float(data["bb_mid"].iloc[-1], np.nan),
        "bb_upper": _safe_float(data["bb_upper"].iloc[-1], np.nan),
        "bb_lower": _safe_float(data["bb_lower"].iloc[-1], np.nan),
        "prev_close": _safe_float(prev.get("close"), np.nan),
        "prev_high": _safe_float(prev.get("high"), np.nan),
        "prev_low": _safe_float(prev.get("low"), np.nan),
        "prev_bb_upper": _safe_float(data["bb_upper"].iloc[-2], np.nan),
        "prev_bb_lower": _safe_float(data["bb_lower"].iloc[-2], np.nan),
        "atr_position": int(position.iloc[-1]) if pd.notna(position.iloc[-1]) else 0,
    }
    required = ["close", "bb_mid", "bb_upper", "bb_lower", "prev_close", "prev_high", "prev_low", "prev_bb_upper", "prev_bb_lower"]
    if not all(np.isfinite(v[k]) for k in required):
        return result

    reasons = []
    if signal == "call":
        valid = (
            v["ema_bullish"] and v["atr_position"] == 1
            and PRECISION_MIN_RSI_CALL <= v["rsi"] <= PRECISION_MAX_RSI_CALL
            and v["prev_low"] <= v["prev_bb_lower"] * 1.001
            and v["close"] > v["prev_close"]
            and v["close"] < v["bb_mid"]
        )
        if not v["ema_bullish"]: reasons.append("CALL contra tendencia EMA")
        if v["atr_position"] != 1: reasons.append("ATR no alcista")
        if not PRECISION_MIN_RSI_CALL <= v["rsi"] <= PRECISION_MAX_RSI_CALL: reasons.append("RSI CALL fuera de rango")
        if not v["prev_low"] <= v["prev_bb_lower"] * 1.001: reasons.append("sin rechazo de banda inferior")
        if not v["close"] > v["prev_close"]: reasons.append("vela no alcista")
        if not v["close"] < v["bb_mid"]: reasons.append("CALL extendido sobre la media")
    else:
        valid = (
            v["ema_bearish"] and v["atr_position"] == -1
            and PRECISION_MIN_RSI_PUT <= v["rsi"] <= PRECISION_MAX_RSI_PUT
            and v["prev_high"] >= v["prev_bb_upper"] * 0.999
            and v["close"] < v["prev_close"]
            and v["close"] > v["bb_mid"]
        )
        if not v["ema_bearish"]: reasons.append("PUT contra tendencia EMA")
        if v["atr_position"] != -1: reasons.append("ATR no bajista")
        if not PRECISION_MIN_RSI_PUT <= v["rsi"] <= PRECISION_MAX_RSI_PUT: reasons.append("RSI PUT fuera de rango")
        if not v["prev_high"] >= v["prev_bb_upper"] * 0.999: reasons.append("sin rechazo de banda superior")
        if not v["close"] < v["prev_close"]: reasons.append("vela no bajista")
        if not v["close"] > v["bb_mid"]: reasons.append("PUT extendido bajo la media")

    result.setdefault("analysis", {})["precision_filter"] = {
        "valid": bool(valid), "signal": signal, "rsi": v["rsi"],
        "atr_position": v["atr_position"], "ema_bullish": v["ema_bullish"],
        "ema_bearish": v["ema_bearish"], "reasons": reasons,
    }
    if not valid:
        result.update({"signal": None, "blocked": True, "entry_type": None, "entry_quality": 0})
        result["reason"] = "Bloqueada por filtro de precisión: " + "; ".join(reasons)
    return result
...
def analyze_market(
    ...
    result = _apply_precision_filter(
        result=result, df=df, candle_1m=candle_1m, previous_m1=previous_m1
    )
